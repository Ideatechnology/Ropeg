...
<?php 
	...
	require_once ("./act_genedu.php");

	global $fLib;
	
	$thn = $fLib->getValue ("select distinct(thn) from fact_edu order by thn desc limit 0,1");
	$male = $fLib->getValue ("select sum(jml) from fact_edu where sex=1 and thn=$thn");
	$fema = $fLib->getValue ("select sum(jml) from fact_edu where sex=2 and thn=$thn");
	$tota = $fLib->getValue ("select sum(jml) from fact_edu where thn=$thn");
