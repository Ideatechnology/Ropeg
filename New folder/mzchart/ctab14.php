<h3>Jumlah PNS Menurut Jenis Kepegawaian dan Jenis Kelamin</h3>

<table width="80%px" cellpadding="3px" class="tbbrowse" border=1>
	<tr class="brheader" align="center">
		<td align="center" width="350px"><b>JENIS KEPEGAWAIAN</b></td>
	...
	<?php 
		...
		$thn = $fLib->getValue ("select distinct(yr) from fact_jpeg order by yr desc limit 0,1");
		$male = $fLib->getValue ("select sum(jml) from fact_jpeg where sex=1 and yr=$thn");
		$fema = $fLib->getValue ("select sum(jml) from fact_jpeg where sex=2 and yr=$thn");
		$tota = $fLib->getValue ("select sum(jml) from fact_jpeg where yr=$thn");
		
		$SQL = "select jpeg, sum(male) as male, sum(fema) as fema, sum(tot) as tot
				from (select jpeg, 
		...
		while ($row = $ret->fetch_array () ) {
			...
			echo "<tr>";
			echo "<td align=\"center\"> " . $fLib->getValue ("select name from dim_jpeg where jpeg='" . $row['jpeg'] . "'") . " </td>";
