<h3>Jumlah PNS Menurut Jenis Jabatan dan Jenis Kelamin</h3>
...
<table width="50%px" cellpadding="3px" class="tbbrowse" border=1>
	...
	<?php 
		...
		$male = $fLib->getValue ("select sum(jml) from fact_peg where sex=1 and yr=$thn");
		$fema = $fLib->getValue ("select sum(jml) from fact_peg where sex=2 and yr=$thn");
		$tota = $fLib->getValue ("select sum(jml) from fact_peg where yr=$thn");
