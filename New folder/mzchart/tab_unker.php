<table width="80%px" cellpadding="3px" class="headerrow" border=1>
	...
	<?php 
		...
		$male = $fLib->getValue ("select sum(jml) from fact_uker where sex=1 and thn=$thn");
		$fema = $fLib->getValue ("select sum(jml) from fact_uker where sex=2 and thn=$thn");
		$tota = $fLib->getValue ("select sum(jml) from fact_uker where thn=$thn");
