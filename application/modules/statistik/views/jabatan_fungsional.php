...
swfobject.embedSWF(
  ...
  {"data-file":"<?php echo site_url()."statistik/json_fungsional";?>"}
  );
...
<div class="col-md-12">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Distribusi PNS Menurut Jabatan Fungsional</li>
    </ol>
...
<h1>Distribusi PNS Menurut Jabatan Fungsional</h1>
...
<?php foreach($jabatan_fungsional as $jabatan_fungsional_row): 

	//male
		if ($jabatan_fungsional_row['male'] == '') { $mal = 0; } 
		else { $mal = $jabatan_fungsional_row['male']; }
		if ($male == 0 or $mal == 0) { $vmal = 0; }
		else {$vmal = ($mal/$male) * 100 ; $vmal = number_format($vmal,2);}
		
		//female
		if ($jabatan_fungsional_row['fema'] == '') { $fem = 0; } 
		else { $fem = $jabatan_fungsional_row['fema']; }
		if ($fema == 0 or $fem == 0) { $vfem = 0; }
		else {$vfem = ($fem/$fema) * 100 ; $vfem = number_format($vfem,2);}
		
		//total
		if ($jabatan_fungsional_row['tot'] == '') { $tot = 0; } 
		else { $tot = $jabatan_fungsional_row['tot']; }
		if ($tota == 0 or $tot == 0) { $vtot = 0; }
		else {$vtot = ($tot/$tota) * 100 ; $vtot = number_format($vtot,2);}

		$dim_funk = $this->database_two->query("select name from dim_funk where funk='" . $jabatan_fungsional_row['funk'] . "'")->row_array();

?>
<tr class="brdata2">
	<td align="center"> <?php echo $dim_funk["name"];?> </td>
	<td align="right"> <?php echo number_format($jabatan_fungsional_row['male']);?></td>
	<td align="right"> <?php echo $vmal;?></td>
	<td align="right"> <?php echo number_format($jabatan_fungsional_row['fema']);?> </td>
	<td align="right"> <?php echo $vfem;?> </td>
	<td align="right"> <?php echo number_format($jabatan_fungsional_row['tot']);?> </td>
